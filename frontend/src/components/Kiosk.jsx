...
function Kiosk({ onAddToCart, cart }) {  
  const [allFoods, setAllFoods] = useState([]); // State for all foods
  ...
  useEffect(() => {
    fetch("http://localhost:8080/api/products/getAllProducts")
      ...
      .then((data) => setAllFoods(data)); // Set categories from API response
  }
  , []); 
  ...
  useEffect(() => {
    ...
    if (category) {
      const filteredFoods = allFoods.filter(
        (food) => food.category.categoryId === parseInt(category)
      );
